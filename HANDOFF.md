...
1. **Cloud sync** — ✅ CODE COMPLETE (sync-code + Upstash Redis), last-write-wins, push-on-change (5s debounce) + pull-on-launch. Backend endpoints in `server.js`: `POST /api/sync/init` (mint 8-char code, seed state), `POST /api/sync/push`, `GET /api/sync/pull/:code` — all return 503 until Upstash env vars exist. Uses Upstash REST API directly (no SDK) via `fetch`; key = `cfa:sync:<CODE>`, 1-year TTL. Frontend: `StoredState.syncCode/syncSavedAt`, hook actions `initSync/joinSync/unlinkSync/forcePush` + `cloudSyncStatus/cloudSyncAt`, two effects in `useStudyCompanion` (pull-on-hydrate, debounced push). UI = `src/components/CloudSyncCard.tsx` on the Overview tab (under Study setup). syncCode/syncSavedAt are stored *inside* the synced blob but always overridden by the local code on read (so a device keeps its own code). **REMAINING (manual, user must do):** (a) create a free Upstash Redis DB at console.upstash.com, (b) copy its `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` from the REST API section, (c) add both as env vars on the Render service (triggers redeploy). Then in-app: Overview → Cloud sync & backup → Start sync → enter the code on iPad/web.
...
## Decisions log
- Local-first; custom hook (no Redux). Backend keeps API key off device. **Cost: all tasks now run on Sonnet** (generation switched Opus→`claude-sonnet-4-6`, ~5x cheaper; override with Render env `CLAUDE_GENERATE_MODEL=claude-opus-4-8` if you ever want top-tier). `runClaude` marks the system prompt as an ephemeral **prompt-cache** breakpoint (repeat calls bill system at ~10%). Reviews = short targeted quizzes (not 40-50 grind). Streak = small achievable daily goal (any real study action), not clearing the whole backlog. Weekly plan should mirror the user's real (synced) chapters. Web version preferred over paid Apple build for "any device, free".
